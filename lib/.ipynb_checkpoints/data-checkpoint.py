...
class IncrementalDataset:
    ...
    def new_task(self, memory=None):
        ...
        train_loader = self._get_loader(x_train, y_train,index_train, mode="train", reverse=True)
        #val_loader = self._get_loader(x_val, y_val,index_val, mode="train") if len(x_val) > 0 else None
        #val_loader = self._get_loader(x_train, y_train,index_train, mode="train", uniform=True)
        val_loader = self._get_loader(x_test, y_test,index_test, mode="test", one_hot_target=True)
    ...
    def _get_loader(self, x, y, index, shuffle=True, mode="train", reverse=False, uniform=False, one_hot_target=False):
        if mode == "train":
        ...
        if reverse:
            return DataLoader(
                Reverse_Dataset(x, y,index, trsf, self.dataset_name, sum(self.increments[:self._current_task])),
            ...
            return DataLoader(
                Uniform_Dataset(x, y,index, trsf, self.dataset_name),
                batch_size=self._batch_size,
                shuffle=shuffle,
                num_workers=self._workers
            )
            ...
            return DataLoader(
                One_hot_Dataset(x, y,index, trsf, self.dataset_name),
                batch_size=self._batch_size,
                shuffle=shuffle,
                num_workers=self._workers
            )
...
class One_hot_Dataset(torch.utils.data.Dataset):

    def __init__(self, x, y,index, trsf, dataset_name):
        self.x, self.y, self.index = x, y,index
        self.trsf = trsf
        self.cls_num = len(set(self.y))
        self.dataset_name = dataset_name
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name == "imagenet":
            ...
            x = Image.fromarray(x)
        x = self.trsf(x)
        y = torch.zeros(self.cls_num).scatter_(dim=0, index=torch.LongTensor([y]), value=1.)
        return x, y, index


class Reverse_Dataset(torch.utils.data.Dataset):

    def __init__(self, x, y, index, trsf, dataset_name, cls_num):
        self.x, self.y, self.index = x, y,index
        self.trsf = trsf
        self.dataset_name = dataset_name
        # print("y: ", y)
        ...
        # print("cls_num: ", self.cls_num)
        # print("img_num_per_cls: ", self.img_num_per_cls)
    ...
    def get_weight(self):
        max_num = max(self.img_num_per_cls)
        class_weight = [max_num / i for i in self.img_num_per_cls]
        # class_weight = [i / max_num for i in self.img_num_per_cls]
        sum_weight = sum(class_weight)
        return class_weight, sum_weight

    def get_class_dict(self):
        class_dict = dict()
        for i, cat_id in enumerate(self.y):
    ...
    def get_img_num_per_cls(self):
        ...
        if self.cls_num == 10:
            old_class = 0
            new_class = self.cls_num
        else:
            old_class = self.cls_num - 10
            new_class = 10
        
        for cls_idx in range(old_class):
            img_num_per_cls.append(int(2000//old_class))
        for cls_idx in range(new_class):
            img_num_per_cls.append(int(500))
    ...
    def __getitem__(self, idx):
        ...
        sample_img, sample_label = self.x[sample_index], self.y[sample_index]
        ...
        y = torch.zeros(self.cls_num).scatter_(dim=0, index=torch.LongTensor([y]), value=1.)
        sample_label = torch.zeros(self.cls_num).scatter_(dim=0, index=torch.LongTensor([sample_label]), value=1.)
        
        meta['sample_image'] = sample_img
        meta['sample_label'] = sample_label

        # weight = np.random.beta(2, 5, 1)
        # weight = np.array([0.5])
        #x_weight = torch.from_numpy(weight.reshape(1, 1, 1)).type(torch.FloatTensor)
        #y_weight = torch.from_numpy(weight.reshape(1)).type(torch.FloatTensor)
        #sample_img = sample_img.type(torch.FloatTensor)
        #meta['sample_image'] = x * (1 - x_weight) + sample_img * x_weight
        #meta['sample_label'] = y * (1 - y_weight) + sample_label * y_weight
        #meta['sample_image'] = meta['sample_image'].type(torch.FloatTensor)
        #meta['sample_label'] = meta['sample_label'].type(torch.FloatTensor)
        # print(meta['sample_label'])

        return x, y, index, meta


class Uniform_Dataset(torch.utils.data.Dataset):
